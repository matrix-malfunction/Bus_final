        ...
        switch (data.type) {
          ...
          case "BUS_UPDATE":
            ...
            const incomingIds = new Set(
              ...
              (data.buses || []).map(function(b) { return b.busId; })

            );

            Object.keys(window.busMarkers).forEach(function(busId) {

              if (!incomingIds.has(busId)) {

                send("BUS_UPDATE: removing stale bus " + busId);
...
const HomeScreen = () => {
  ...
  const {
    ...
    followBusId,
    setFollowBusId,
    setUserLocation: setBusContextUserLocation
  } = useBus();
  ...
  useEffect(() => {
    ...
    retryTimeoutRef.current = setTimeout(() => {
      ...
      const latestActiveBuses = latestBusesArray.filter(
        (bus) =>
          bus &&
          bus.busId &&
          bus.lat &&
  ...
  // Active buses for "Buses Around You" section
        ...
        <View style={styles.busesSection}>
          ...
          {activeBuses.length === 0 ? (
            <View style={styles.emptyBusCard}>
              <Text style={styles.emptyBusText}>
            ...
            <ScrollView
              ...
              contentContainerStyle={styles.busesScrollContent}
            >
              {activeBuses.map(bus => (
                <TouchableOpacity
                  key={bus.busId}
                  ...
                  onPress={() => {
                    console.log(
                      "[RN HomeScreen] Bus selected:",
                      bus.busId
                    );
                    setFollowBusId(bus.busId);
                    navigation.navigate("FullMap");
                  }}
                >
                  ...
                  <Text style={styles.busNumber} numberOfLines={1}>
                    {bus.busNumber || bus.routeNumber || "Campus Bus"}
                  </Text>
                  <Text style={styles.busRoute} numberOfLines={1}>
                    {bus.routeName || "Active Route"}
                  </Text>
            ...
            nearestStops.map((stop, index) => (
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  console.log(
                    "[RN HomeScreen] Route requested:",
                    normalizedStop.name
                  );

                  setSelectedStopRoute(normalizedStop);
                  navigation.navigate("FullMap");
