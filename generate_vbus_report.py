...
# Helper to set paragraph formatting
...
def add_table_custom(rows, cols, header_texts=None):
...
table = add_table_custom(rows=16, cols=2)
table.rows[0].cells[0].text = 'Abbreviation'
table.rows[0].cells[1].text = 'Full Form'
for paragraph in table.rows[0].cells[0].paragraphs: paragraph.runs[0].bold = True
for paragraph in table.rows[0].cells[1].paragraphs: paragraph.runs[0].bold = True
abbrs = [
...
for i, (abbr, full) in enumerate(abbrs, start=1):
    table.rows[i].cells[0].text = abbr
    table.rows[i].cells[1].text = full
if len(abbrs) + 1 > len(table.rows):
    for _ in range(len(abbrs) + 1 - len(table.rows)):
        table.add_row()
    for i, (abbr, full) in enumerate(abbrs, start=1):
        table.rows[i].cells[0].text = abbr
        table.rows[i].cells[1].text = full

# Rebuild abbreviation table properly
doc.add_paragraph()
add_heading_custom('LIST OF ABBREVIATIONS', level=1)
